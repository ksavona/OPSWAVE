...
- Select OpenAI as the first optional live provider and the Responses API for structured extraction.
...
- Use the platform `fetch` boundary for the small live adapter so the provider remains replaceable without an otherwise-unused production SDK.
...
- Live adapter tests use a fake transport and cover structured normalization, bounded values, refusal/error handling, and credential validation without paid calls.
